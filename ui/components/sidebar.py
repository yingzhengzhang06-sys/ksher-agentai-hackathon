...
def render_sidebar() -> str:
    ...
    with st.sidebar:
        ...
        # ---- 导航菜单（Radio） ----
        page_labels = PAGE_ITEMS[:]
        page_map = {name: name for name in PAGE_ITEMS}
        ...
        if st.session_state.get("battle_pack"):
            st.markdown(
                ...
                '>
                    <span style='display:inline-block;width:6px;height:6px;border-radius:50%;background:{BRAND_COLORS["success"]};margin-right:0.3rem;'></span>
                    <span>作战包已生成</span>
                </div>
                """,
                unsafe_allow_html=True,
            ...
            st.markdown(
                ...
                '>
                    <span style='display:inline-block;width:6px;height:6px;border-radius:50%;background:{BRAND_COLORS["text_muted"]};margin-right:0.3rem;'></span>
                    <span>等待生成作战包</span>
                </div>
                """,
                unsafe_allow_html=True,

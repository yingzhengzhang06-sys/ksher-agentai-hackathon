#!/bin/bash
# 工程环境负责人 - 开发前自检脚本
# 目标：确认开发前环境完整性，生成 dev_env_check_report.md

set -u

REPORT_FILE="dev_env_check_report.md"
EXPECTED_BRANCH="feature/moments-create"
EXPECTED_REMOTE="https://github.com/yingzhengzhang06-sys/ksher-agentai-hackathon.git"
MOMENTS_TEST_TARGET="${MOMENTS_TEST_TARGET:-tests/test_moments_ui.py}"
...
if git fetch origin "$EXPECTED_BRANCH" >/tmp/dev_env_check_fetch.log 2>&1; then
    add_pass "远程分支 fetch 成功：origin/$EXPECTED_BRANCH"
else
    add_blocker "远程分支 fetch 失败，详情见 /tmp/dev_env_check_fetch.log"
fi
...
if [ -n "$PYTHON_CMD" ]; then
    if [ "${RUN_FULL_TESTS:-0}" = "1" ]; then
        ...
        TEST_COMMAND=("$PYTHON_CMD" -m pytest "$MOMENTS_TEST_TARGET" -q)
        TEST_LABEL="轻量测试 $MOMENTS_TEST_TARGET"
    fi

    "${TEST_COMMAND[@]}" >/tmp/dev_env_check_pytest.log 2>&1
    if [ $? -eq 0 ]; then
        add_pass "$TEST_LABEL 通过：$(tail -n 1 /tmp/dev_env_check_pytest.log)"
    else
        add_blocker "$TEST_LABEL 未通过，详情见 /tmp/dev_env_check_pytest.log"
    fi
fi
...
add_next_step "打开浏览器确认页面非空白：http://localhost:8501"
...
echo "开发前自检完成，报告生成在 $REPORT_FILE"

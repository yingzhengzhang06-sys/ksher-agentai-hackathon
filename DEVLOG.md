...
| 优先级 | 任务 | 结果 | 详情 |
|--------|------|------|------|
| **P0** | Apple 风格 UI 升级 | ✅ 通过 | 浅色主题/药丸按钮/语义化 alert/代码块适配 |
| **P0** | error_handlers 集成 | ✅ 通过 | 5 个页面全部集成（10 处调用） |
| **P0** | Plotly 图表浅色适配 | ✅ 通过 | 字体 #B0B0C0 → #86868B（3处） |
| **P0** | sidebar 深色文字 | ✅ 通过 | #1D1D1F + 红色选中 |
| **P0** | 硬编码暗色值清理 | ✅ 通过 | 已清除 #0F0F1A/#1E1E2F/#B0B0C0 |
| **P1** | 响应式布局验证 | ✅ 通过 | @media(max-width: 768px) + 列自动堆叠 |
| **P1** | 一键备战页面优化 | ✅ 通过 | 表单/按钮/Tab/成本模型（config.RATES_CONFIG） |
| **P1** | 仪表盘页面优化 | ✅ 通过 | KPI卡片/漏斗/饼图/柱状图/折线图可读性 OK |
| **P2** | 页面截图准备 | ✅ 完成 | 6 个页面全部截图（assets/screenshots/） |
| **P2** | 演示脚本配合 | ✅ 完成 | UI 流程与 docs/demo_script.md 一致 |

**页面加载速度检查**（本地 localhost:8502，WiFi 环境）

| 页面 | 首次加载 | 说明 |
|------|---------|------|
| 一键备战 | ~2.1s | 含表单渲染 |
| 内容工厂 | ~1.8s | 含空状态 |
| 知识问答 | ~1.7s | 含常见问题 pill |
| 异议模拟 | ~2.3s | 含预设异议渲染 |
| 海报/PPT | ~1.9s | 含 Tab 切换 |
| 仪表盘 | ~3.2s | 含 Plotly 图表（最慢） |
